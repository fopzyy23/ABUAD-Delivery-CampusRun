...
| `20261003_product_availability_ui.sql` | Per-product Available/Not available UI support: `products_select_public` relaxed to `USING (true)` so vendors can toggle `products.active` OFF and the item stays visible (marked "Currently unavailable", non-orderable) instead of vanishing (purchase security unchanged — `place_order` + `enforce_order_item_pricing` still reject inactive products); `products` added to the `supabase_realtime` publication so availability flips propagate to the customer app live (pattern from 20260903). No new columns. | yes |
| `20261004_auto_settlement_on_delivered.sql` | L-B1: automatic settlement generation. Extracts the 20260930 `generate_settlement` body into `_settle_order_core(uuid)` (SECURITY DEFINER, EXECUTE locked to owner + service_role), re-creates `generate_settlement` as an admin wrapper (same is_admin gate/grants), and adds SECURITY DEFINER trigger function `auto_settle_delivered_order()` + `AFTER UPDATE OF status` / `AFTER UPDATE OF payment_status` triggers on orders — an order that becomes Delivered + paid is settled idempotently (unique settlement constraints + FOR UPDATE + count pre-check); failures degrade to WARNING and can never corrupt the delivery transition. No money moves. |
| `20261005_secure_withdrawal_rpc.sql` | H-1: server-authoritative withdrawals. New SECURITY DEFINER `request_withdrawal(numeric)` (auth.uid() rider ownership + FOR UPDATE, approved-rider gate, amount > 0, authoritative available balance from `delivery_settlements.rider_amount` minus ALL non-rejected requests, atomic insert). `get_rider_earnings` encumbrance aligned to the same rule. Direct client INSERT closed: `withdrawal_requests_insert_own` policy dropped + INSERT revoked from authenticated — the RPC is the only creation path. Admin approval/payment workflow unchanged. |
| `20261023_withdrawal_bank_details.sql` | Action 10 extension: add `account_name`, `account_number`, `bank_name`, `bank_code` to `withdrawal_requests` (idempotent `ALTER TABLE` + an `account_number` format CHECK). Rebuilds `request_withdrawal` into a mandatory 5-argument signature (`p_amount`, `p_account_name`, `p_account_number`, `p_bank_name`, `p_bank_code`) that validates bank details server-side with regexes mirroring `paystack-transfer-recipient` (account_number `^[0-9]{6,20}$`, bank_code `^[A-Za-z0-9]{2,10}$`, names ≤120 chars); the legacy single-argument signature is dropped so a bank-less withdrawal can never be submitted. Preserves the 20261022 lifetime-earnings boundary (`ds.status <> 'reversed'`) and encumbrance rule. Rider withdrawal form + RPC carry payout bank details; recipient registration is best-effort via the rider-JWT Identity Edge Function. Admin review UI gains a Bank account column. No money moves (record-only). |
