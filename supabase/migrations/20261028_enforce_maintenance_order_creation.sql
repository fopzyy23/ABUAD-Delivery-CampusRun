-- Enforce global maintenance mode at the authoritative order-creation RPCs.
-- The checks are deliberately after caller authentication and before any
-- admission validation, order insert, payment state, or delivery side effect.
...
BEGIN
  ...
  IF COALESCE((SELECT maintenance_mode FROM public.site_settings WHERE id = 1), false) THEN
    RAISE EXCEPTION 'Dropzyy is currently under maintenance';
  END IF;
